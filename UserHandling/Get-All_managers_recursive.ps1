				...
				IF ($PSBoundParameters['Recurse'])
				{
					...
					ForEach-Object -Process {
						$_.directreports | ForEach-Object -Process {
							...
							Get-ADUser -Identity $PSItem -Properties mail, manager,enabled | where samaccountname -NotLike 'z*' | Select-Object -Property Name, SamAccountName, Mail, @{ Name = "Manager"; Expression = { (Get-Aduser -identity $psitem.manager).samaccountname } },enabled
...
$name = "qinghua.peng"
Get-ADDirectReports -Identity $name -Recurse | export-csv -Path c:\temp\$name.csv -NoTypeInformation -Encoding Unicode -Append
